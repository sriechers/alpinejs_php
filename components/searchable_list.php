  ...
  <div class="mt-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
    <template x-for="item in filteredItems" :key="item.id">
      <div
        ...
        <div class="text-sm">
          <p class="text-gray-900 leading-none" x-text="item.employee_name + ' (' + item.employee_age + ')'"></p>
          <p class="text-gray-600" x-text="'$'+item.employee_salary/100"></p>
        </div>
      </div>
    </template>
  </div>
...
function data<?=$_UNIQUE_ID?>() {
  return {
    ...
    get filteredItems() {
      ...
      return this.items.filter((item) => item.employee_name
        .toLowerCase()
